...
function NtlmClient(credentials, AxiosConfig) {
    ...
    client.interceptors.response.use(function (response) {
        ...
        if (error && error.status === 401
            ...
            var t1Msg = ntlm.createType1Message(credentials.workstation, credentials.domain, authenticationType);
            ...
            && error.headers['www-authenticate'].length > 50
            && error.headers['www-authenticate'].includes(authenticationType)) {
            var t2Msg = ntlm.decodeType2Message((error.headers['www-authenticate'].match(new RegExp("^" + authenticationType + "\\s+(.+?)(,|\\s+|$)")) || [])[1], authenticationType);
            var t3Msg = ntlm.createType3Message(t2Msg, credentials.username, credentials.password, credentials.workstation, credentials.domain, authenticationType);
