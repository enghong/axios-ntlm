...
function createType1Message(workstation, target, authenticationType) {
    if (authenticationType === void 0) { authenticationType = "NTLM"; }
    ...
    return "".concat(authenticationType, " ") + buf.toString('base64', 0, dataPos);
}
function decodeType2Message(str, authenticationType) {
    if (authenticationType === void 0) { authenticationType = "NTLM"; }
    if (str === undefined) {
    ...
    var ntlmMatch = new RegExp("^".concat(authenticationType, " ([^,\\s]+)")).exec(str);
    if (ntlmMatch) {
        str = ntlmMatch[1];
    }
...
function createType3Message(type2Message, username, password, workstation, target, authenticationType) {
    if (authenticationType === void 0) { authenticationType = "NTLM"; }
